        ...
        for (const auto& nodePtr : nodes) {
            if (!nodePtr) {
                 ...
                 // Write placeholder data? Need to handle this on load if nulls are possible
                  int dummy_id = -1; float dummy_float = 0.0f; int dummy_int = 0;
...
bool loadGraphFromBinary(const std::string& filename,
    ...
    try {
        ...
        if (infile.fail() || node_count == 0 || node_count > 50000000) { // Basic sanity check on count
        ...
        for (uint64_t i = 0; i < node_count; ++i) {
            ...
            // Handle potential placeholder for nullptrs if needed
             if (id == -1) { // Assuming -1 was placeholder used in save
                 nodes[i] = nullptr;
                 continue;
             }

            nodes[i] = std::make_unique<Node>(id, length, begin_x, begin_y, 0, 0); // Use dummy end coords
            if (!nodes[i]) { // Check make_unique result
        ...
        infile.read(reinterpret_cast<char*>(&adj_parent_count), sizeof(adj_parent_count));
         if (infile.fail() || adj_parent_count > 50000000) { // Basic sanity check
...
int main(int argc, char *argv[])
{
    ...
    if (!loadedFromCache) {
        ...
        if (slurm_cpus_var != nullptr) { try { int c = std::stoi(slurm_cpus_var); if (c > 0) num_threads = c; } catch (...) {} }
        ...
        while (nodes_parsed_count.load() < num_nodes) { std::this_thread::sleep_for(std::chrono::milliseconds(200)); }
        for (auto& t : node_threads) { t.join(); }
        std::cout << "Populating node map..." << std::endl;
        for (const auto& nodePtr : allNodes) {
            if (nodePtr) { nodeMap[nodePtr->getId()] = nodePtr.get(); }
        }
        ...
        if (!adjInfile.is_open()) { std::cerr << "Error re-opening device file for adjacency." << std::endl; return 1; }
        adjInfile.seekg(adjStartPosition);
        if (!adjInfile) { std::cerr << "Error seeking to adjacency list position." << std::endl; return 1; }
        while (std::getline(adjInfile, line)) { adjLines.push_back(line); }
        adjInfile.close();
        ...
        while (adj_parsed_count.load() < adjLines.size()) { std::this_thread::sleep_for(std::chrono::milliseconds(200)); }
        for (auto& t : adj_threads) { t.join(); }
        ...
        for (const auto& local_map : local_adjs) {
            for (auto const& [parentId, childrenVec] : local_map) { // Use different name
    ...
    if (!parseSuccess) { // Double check just in case
    ...
    for (auto const& [id, nodePtr] : nodeMap) {
        if (nodePtr) nodePtr->resetOccupancy();
    }
    std::set<int> netsToRoute;
    for (int i = 0; i < nets.size(); ++i) netsToRoute.insert(i);
    int iteration;
    ...
    for (iteration = 0; iteration < MAX_ITERATIONS; ++iteration) {
        ...
        for (int netIdx : netsToRoute) {
            if (netIdx >= nets.size()) continue;
            Net& net = nets[netIdx];
            if (!net.route.empty()) { // Only decrement if it had a route
                std::unordered_set<int> nodesInOldPath;
                 for (const auto& edge : net.route) {
                     if (nodeMap.count(edge.first)) nodesInOldPath.insert(edge.first);
                     if (nodeMap.count(edge.second)) nodesInOldPath.insert(edge.second);
                 }
        ...
        */
        ...
        auto reroute_worker = [&](int start_chunk_idx, int end_chunk_idx) {
            for (int i = start_chunk_idx; i < end_chunk_idx; ++i) {
                ...
                if (netIdx >= nets.size()) continue; // Safety check
                    ...
                    for (int sink : net.sinks) {
                        ...
                        if (!path.empty()) {
                            for (size_t p = 0; p < path.size() - 1; ++p) {
                                ...
                                if (nodeMap.count(path[p])) currentNetNodes.insert(path[p]);
                                if (nodeMap.count(path[p + 1])) currentNetNodes.insert(path[p + 1]);
                            }
                            currentRoutedSinks++;
                        } else {
                            netSuccess = false; 
        ...
        for (int i = 0; i < nets.size(); ++i) {
             ...
             if (!needsReroute && !net.route.empty()) { // Check only if currently routed
                 std::unordered_set<int> nodesInFinalPath; // Recalculate nodes for its current route
                 for(const auto& edge : net.route) {
                      if (nodeMap.count(edge.first)) nodesInFinalPath.insert(edge.first);
                      if (nodeMap.count(edge.second)) nodesInFinalPath.insert(edge.second);
                 }
    ...
    for (const auto& net : nets) {
        ...
        if (fullyRouted) {
            ...
            for (const auto& edge : net.route) {
                 if(nodeMap.count(edge.first)) nodesInNet.insert(edge.first);
                 if(nodeMap.count(edge.second)) nodesInNet.insert(edge.second);
            }
